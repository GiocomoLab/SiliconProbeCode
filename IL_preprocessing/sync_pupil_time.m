...
pupilData_name = fullfile(root, experiment_name, mouse_name, 'videos',...
    strcat(session_name,'_pupilData.mat'));
pupilTimes_name = fullfile(root, experiment_name, mouse_name, 'videos',...
    strcat(session_name,'_framedata.mat'));
vrData_name = fullfile(root, experiment_name, mouse_name, 'VR',...
    strcat(session_name,'_position.txt'));
...
% fclose(fpNIDAQ);
%% verify alignment, unfortunately, the framedata.times seems to be lacking the last few frames
