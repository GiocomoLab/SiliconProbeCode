function [pupil, whisk, vid_start_idx] = analyzeFaceVideo(mouse, session, frame_idx)
...
p = readtable('G:\My Drive\Giocomo Lab\RandomForage\UniversalParams.xlsx');
baseFolder = p.WorkingDirectory{1};
vid = VideoReader([baseFolder mouse '\Video\' session '.mp4']);
vid_dir = [baseFolder 'Analysis\' mouse '\VR\video\' session '\'];
...
for i = 1:max(frame_idx)
  ...
  if i == frame_idx(1)
    ...
    % draw whisker ROI around whisker pad
...
time = toc;
disp('min to extract pupil/whisk data:')
disp(time/60)
